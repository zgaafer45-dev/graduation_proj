// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from sabry_hardware:srv/ChangeTool.idl
// generated code does not contain a copyright notice
#include "sabry_hardware/srv/detail/change_tool__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `tool_name`
#include "rosidl_runtime_c/string_functions.h"

bool
sabry_hardware__srv__ChangeTool_Request__init(sabry_hardware__srv__ChangeTool_Request * msg)
{
  if (!msg) {
    return false;
  }
  // tool_name
  if (!rosidl_runtime_c__String__init(&msg->tool_name)) {
    sabry_hardware__srv__ChangeTool_Request__fini(msg);
    return false;
  }
  return true;
}

void
sabry_hardware__srv__ChangeTool_Request__fini(sabry_hardware__srv__ChangeTool_Request * msg)
{
  if (!msg) {
    return;
  }
  // tool_name
  rosidl_runtime_c__String__fini(&msg->tool_name);
}

bool
sabry_hardware__srv__ChangeTool_Request__are_equal(const sabry_hardware__srv__ChangeTool_Request * lhs, const sabry_hardware__srv__ChangeTool_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // tool_name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->tool_name), &(rhs->tool_name)))
  {
    return false;
  }
  return true;
}

bool
sabry_hardware__srv__ChangeTool_Request__copy(
  const sabry_hardware__srv__ChangeTool_Request * input,
  sabry_hardware__srv__ChangeTool_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // tool_name
  if (!rosidl_runtime_c__String__copy(
      &(input->tool_name), &(output->tool_name)))
  {
    return false;
  }
  return true;
}

sabry_hardware__srv__ChangeTool_Request *
sabry_hardware__srv__ChangeTool_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Request * msg = (sabry_hardware__srv__ChangeTool_Request *)allocator.allocate(sizeof(sabry_hardware__srv__ChangeTool_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(sabry_hardware__srv__ChangeTool_Request));
  bool success = sabry_hardware__srv__ChangeTool_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
sabry_hardware__srv__ChangeTool_Request__destroy(sabry_hardware__srv__ChangeTool_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    sabry_hardware__srv__ChangeTool_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
sabry_hardware__srv__ChangeTool_Request__Sequence__init(sabry_hardware__srv__ChangeTool_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Request * data = NULL;

  if (size) {
    data = (sabry_hardware__srv__ChangeTool_Request *)allocator.zero_allocate(size, sizeof(sabry_hardware__srv__ChangeTool_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = sabry_hardware__srv__ChangeTool_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        sabry_hardware__srv__ChangeTool_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
sabry_hardware__srv__ChangeTool_Request__Sequence__fini(sabry_hardware__srv__ChangeTool_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      sabry_hardware__srv__ChangeTool_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

sabry_hardware__srv__ChangeTool_Request__Sequence *
sabry_hardware__srv__ChangeTool_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Request__Sequence * array = (sabry_hardware__srv__ChangeTool_Request__Sequence *)allocator.allocate(sizeof(sabry_hardware__srv__ChangeTool_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = sabry_hardware__srv__ChangeTool_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
sabry_hardware__srv__ChangeTool_Request__Sequence__destroy(sabry_hardware__srv__ChangeTool_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    sabry_hardware__srv__ChangeTool_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
sabry_hardware__srv__ChangeTool_Request__Sequence__are_equal(const sabry_hardware__srv__ChangeTool_Request__Sequence * lhs, const sabry_hardware__srv__ChangeTool_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!sabry_hardware__srv__ChangeTool_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
sabry_hardware__srv__ChangeTool_Request__Sequence__copy(
  const sabry_hardware__srv__ChangeTool_Request__Sequence * input,
  sabry_hardware__srv__ChangeTool_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(sabry_hardware__srv__ChangeTool_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    sabry_hardware__srv__ChangeTool_Request * data =
      (sabry_hardware__srv__ChangeTool_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sabry_hardware__srv__ChangeTool_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          sabry_hardware__srv__ChangeTool_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!sabry_hardware__srv__ChangeTool_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
sabry_hardware__srv__ChangeTool_Response__init(sabry_hardware__srv__ChangeTool_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    sabry_hardware__srv__ChangeTool_Response__fini(msg);
    return false;
  }
  return true;
}

void
sabry_hardware__srv__ChangeTool_Response__fini(sabry_hardware__srv__ChangeTool_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
}

bool
sabry_hardware__srv__ChangeTool_Response__are_equal(const sabry_hardware__srv__ChangeTool_Response * lhs, const sabry_hardware__srv__ChangeTool_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  return true;
}

bool
sabry_hardware__srv__ChangeTool_Response__copy(
  const sabry_hardware__srv__ChangeTool_Response * input,
  sabry_hardware__srv__ChangeTool_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  return true;
}

sabry_hardware__srv__ChangeTool_Response *
sabry_hardware__srv__ChangeTool_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Response * msg = (sabry_hardware__srv__ChangeTool_Response *)allocator.allocate(sizeof(sabry_hardware__srv__ChangeTool_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(sabry_hardware__srv__ChangeTool_Response));
  bool success = sabry_hardware__srv__ChangeTool_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
sabry_hardware__srv__ChangeTool_Response__destroy(sabry_hardware__srv__ChangeTool_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    sabry_hardware__srv__ChangeTool_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
sabry_hardware__srv__ChangeTool_Response__Sequence__init(sabry_hardware__srv__ChangeTool_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Response * data = NULL;

  if (size) {
    data = (sabry_hardware__srv__ChangeTool_Response *)allocator.zero_allocate(size, sizeof(sabry_hardware__srv__ChangeTool_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = sabry_hardware__srv__ChangeTool_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        sabry_hardware__srv__ChangeTool_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
sabry_hardware__srv__ChangeTool_Response__Sequence__fini(sabry_hardware__srv__ChangeTool_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      sabry_hardware__srv__ChangeTool_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

sabry_hardware__srv__ChangeTool_Response__Sequence *
sabry_hardware__srv__ChangeTool_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  sabry_hardware__srv__ChangeTool_Response__Sequence * array = (sabry_hardware__srv__ChangeTool_Response__Sequence *)allocator.allocate(sizeof(sabry_hardware__srv__ChangeTool_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = sabry_hardware__srv__ChangeTool_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
sabry_hardware__srv__ChangeTool_Response__Sequence__destroy(sabry_hardware__srv__ChangeTool_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    sabry_hardware__srv__ChangeTool_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
sabry_hardware__srv__ChangeTool_Response__Sequence__are_equal(const sabry_hardware__srv__ChangeTool_Response__Sequence * lhs, const sabry_hardware__srv__ChangeTool_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!sabry_hardware__srv__ChangeTool_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
sabry_hardware__srv__ChangeTool_Response__Sequence__copy(
  const sabry_hardware__srv__ChangeTool_Response__Sequence * input,
  sabry_hardware__srv__ChangeTool_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(sabry_hardware__srv__ChangeTool_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    sabry_hardware__srv__ChangeTool_Response * data =
      (sabry_hardware__srv__ChangeTool_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!sabry_hardware__srv__ChangeTool_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          sabry_hardware__srv__ChangeTool_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!sabry_hardware__srv__ChangeTool_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
